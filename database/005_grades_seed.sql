...
BEGIN
    ...
    eid := (SELECT id FROM events WHERE event_shortcut = 'ABU-S2-SK');
    ...
    eid := (SELECT id FROM events WHERE class_id = (SELECT id FROM classes WHERE designation = class_name));
    IF eid IS NULL THEN RAISE EXCEPTION 'Event not found for %', class_name; END IF;
    ...
    eid := (SELECT id FROM events WHERE event_shortcut = 'ABU-S1-SK');
    ...
    -- ========================================================================
    -- Image 6: 122 - Abläufe mit einer Scriptsprache ...
    -- ========================================================================
    class_name := '122 - Abläufe mit einer Scriptsprache automatisieren';
    eid := (SELECT id FROM events WHERE class_id = (SELECT id FROM classes WHERE designation = class_name));
    IF eid IS NULL THEN RAISE EXCEPTION 'Event not found for %', class_name; END IF;
    ...
    -- ========================================================================
    class_name := 'Allgemeinbildung Semester 1';
    eid := (SELECT id FROM events WHERE event_shortcut = 'ABU-S1-GES');
